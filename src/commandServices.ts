...
export interface BoardCommandServices {
  inspectWebsite(options: { readonly url: string }): Promise<InspectWebsiteResult>;
}
...
export function createDefaultBoardCommandServices(
  ...
  const fixtureContentType =
    env.ANKHORAGE_BOARD_TEST_WEB_FIXTURE_CONTENT_TYPE ?? 'text/html; charset=utf-8';
  const fixtureStatus = parseFixtureStatus(env.ANKHORAGE_BOARD_TEST_WEB_FIXTURE_STATUS);
