...
import { createBufferedContext, createStubBoardCommandServices } from './testSupport.js';
...
describe('runCli', () => {
  it('prints help with no arguments', async () => {
    ...
    expect(context.stdout).toContain('ankhorage-board manifest generate <source>');
  });
  ...
  it('dispatches manifest generate through the shared runner', async () => {
    const context = createBufferedContext();
    const result = await runCli(['manifest', 'generate', 'website-source'], context);
  ...
  it('rejects extra positional arguments', async () => {
    const context = createBufferedContext();
    const result = await runCli(['web', 'https://example.com', 'extra'], context);
  ...
  it('rejects unsupported flags', async () => {
    const context = createBufferedContext();
    const result = await runCli(['web', 'https://example.com', '--json'], context);
  ...
  it('returns deferred stderr for --create', async () => {
    const context = createBufferedContext();
    const result = await runCli(['web', 'https://example.com', '--create', 'my-app'], context);

    expect(result.exitCode).toBe(1);
    expect(context.stdout).toBe('');
    expect(context.stderr).toContain('Project creation from a web boarding plan is deferred.');
  });
  ...
  it('keeps unknown top-level commands generic', async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(context.stderr).toContain('Unknown board command: https://example.com');
...
describe('package-scoped CLI commands', () => {
  ...
  it('covers `bun src/cli/index.ts web https://example.com --plan` and keeps output identical', () => {
    ...
    const withPlan = Bun.spawnSync(
      [process.execPath, 'src/cli/index.ts', 'web', 'https://example.com', '--plan'],
      {
        cwd,
        env,
        stderr: 'pipe',
