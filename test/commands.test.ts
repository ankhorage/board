...
import { BOARD_COMMANDS, resolveBoardCommand, runBoardCommand } from '../src/commands.js';
...
describe('BOARD_COMMANDS', () => {
  ...
  it('does not include aliases in the bootstrap slice', () => {
    ...
    expect(commandKeys.every((keys) => keys.includes('path') && !keys.includes('aliases'))).toBe(
      true,
    );
  });
});
...
describe('resolveBoardCommand', () => {
  it('resolves the longest matching command path', () => {
    expect(resolveBoardCommand(['manifest', 'generate', 'source'])?.command.path).toEqual([
      'manifest',
      'generate',
    ]);
  });
...
describe('runBoardCommand', () => {
  it('prints deferred stub output for openapi', async () => {
    ...
    const result = await runBoardCommand(BOARD_COMMANDS[1], ['./openapi.json'], context);
  ...
  it('prints plan JSON for a valid website URL', async () => {
    ...
    const result = await runBoardCommand(BOARD_COMMANDS[0], ['https://example.com'], context);
